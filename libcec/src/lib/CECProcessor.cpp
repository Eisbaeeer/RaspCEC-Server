...
{
  m_TCPServer = new CCECTCPServer(this);
  m_TCPServer->CreateThread();
...
CCECTCPServer::CCECTCPServer(CCECProcessor* processor) : m_processor(processor)  {}

void* CCECTCPServer::Process(void)
{
    ...
    while( (new_socket = accept(socket_desc, (struct sockaddr *)&client, (socklen_t*)&c)) )
    {
        ...
        while( (read_size = recv(new_socket , client_message , 100 , 0)) > 0 )
        {
            ...
            {
              bytes.PushBack((uint8_t)client_message[i]);              
            }
            
            m_processor->Transmit(bytes,true);
